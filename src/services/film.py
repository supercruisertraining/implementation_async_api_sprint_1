...
class FilmService:
    ...
    async def get_by_id(self, film_uuid: str) -> Optional[Film]:
        ...
        if not film:
            film = await self.storage.get_object_by_id(
                index=self.name, object_id=film_uuid,
            )
            if not film:
                return None
    ...
    async def search_films(
        self, search_query, page_size: int, page_number: int,
    ) -> List[Film]:
        query_body = {
            "query": {
                "bool": {
                    "should": [
                        {
                            "match": {
                                "title": {"query": search_query, "fuzziness": "auto"},
                            },
                        },
                        {
                            "match": {
                                "description": {"query": search_query, "fuzziness": "1"},
                            },
                        },
                        {
                            "nested": {
        ...
        film_list = await self.storage.get_object_list(
            index=self.name, query=json.dumps(query_body),
        )
    ...
    async def get_film_list(
        ...
        film_list = await self.cache.get_from_cache(
            self.cache.cook_cache_key(
                self.name, page_number, page_size, sort_rule, filters_should,
            ),
        )
        if not film_list:
            film_list = await self.storage.get_object_list(
