...
class GenreService:
    ...
    async def get_genre_by_id(self, genre_id: str) -> Optional[GenreDetail]:
        ...
        if not genre_data:
            genre_data = await self.storage.get_object_by_id(
                index=self.name, object_id=genre_id,
            )
            if genre_data:
                await self.cache.put_to_cache(
                    key=genre_id, value=json.dumps(genre_data),
                )
            else:
                return None
    ...
    async def get_genre_list(self) -> list[Genre]:
        ...
        if not genres_list:
            ...
            if genres_list:
                await self.cache.put_to_cache(
                    key=self.name, value=json.dumps(genres_list),
                )
            else:
                return []
        return [Genre(**genre) for genre in genres_list]
