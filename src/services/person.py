...
class PersonService:
    ...
    async def get_person_by_id(self, person_id: str) -> Optional[Person]:
        ...
        if not person_data:
            person_data = await self.storage.get_object_by_id(
                index=self.name, object_id=person_id,
            )
            if person_data:
                await self.cache.put_to_cache(
                    key=person_id, value=json.dumps(person_data),
                )
            else:
                return None
    ...
    async def get_persons_list(
        self, page_number: int, page_size: int, sort_rule: Optional[dict],
    ) -> List[Person]:
        query_body = {}
        ...
        person_list = await self.cache.get_from_cache(
            self.cache.cook_cache_key(self.name, page_number, page_size, sort_rule),
        )
        if not person_list:
            person_list = await self.storage.get_object_list(
                index=self.name, query=json.dumps(query_body),
            )
            if person_list:
                await self.cache.put_to_cache(
                    self.cache.cook_cache_key(
                        self.name, page_number, page_size, sort_rule,
                    ),
