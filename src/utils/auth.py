...
class PermissionChecker:
    ...
    async def __call__(self, authorization: str = Header()):
        ...
        try:
            ...
            if not user_id:
                raise HTTPException(status_code=HTTPStatus.UNAUTHORIZED, detail="Invalid Authentication token.")
        ...
        try:
            async with ClientSession() as session:
                ...
                async with session.get(url,
                                       params={"resource_role_name": self.minimal_role, "user_id": user_id}) as response:
                    if response.ok:
            ...
            raise HTTPException(status_code=HTTPStatus.FORBIDDEN, detail="Client has no permissions")
