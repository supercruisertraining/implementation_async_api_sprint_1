...
async def get_person_by_id(
    person_uuid: str, person_service: PersonService = Depends(get_person_service),
):
    person_info = await person_service.get_person_by_id(person_uuid)
    if not person_info:
        raise HTTPException(
            status_code=HTTPStatus.NOT_FOUND, detail=PERSON_NOT_FOUND_MESSAGE,
        )
...
async def get_person_list(
    ...
    return [Person(id=x.id, full_name=x.full_name) for x in person_list]
